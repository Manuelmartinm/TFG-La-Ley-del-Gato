// ─────────────────────────────────────────────────────────────
// main2.js — bucle principal del Nivel 2
// Igual que main.js pero carga Nivel2 y redirige a mapa al completar
// ─────────────────────────────────────────────────────────────
...
import {
    MAPA as MAPA_N2,
    ENEMIGOS_CONFIG as ENEMIGOS_N2,
    INICIO_JUGADOR as INICIO_N2,
    PUERTA as PUERTA_N2,
    LLAVE as LLAVE_N2,
    QUESOS_PEQUENOS as QUESOS_P_N2,
    QUESOS_DORADOS as QUESOS_D_N2,
    ESCUDOS as ESCUDOS_N2,
} from './niveles/Nivel2.js';
...
function cargarNivel() {
    mapa        = MAPA_N2.map(fila => [...fila]);
    jugador     = new Jugador(INICIO_N2.x, INICIO_N2.y);
    enemigos    = ENEMIGOS_N2.map(cfg => new Enemigo({ ...cfg, pi: 0 }));
    proyectiles = [];
    proyMortero = [];
    puerta      = PUERTA_N2;
    llaveCogida = false;
    ...
    objetos = [
        new Llave(LLAVE_N2.x, LLAVE_N2.y),
        ...QUESOS_P_N2.map(q => new QuesoPequeno(q.x, q.y)),
        ...QUESOS_D_N2.map(q => new QuesoDorado(q.x, q.y, q.duracion)),
        ...ESCUDOS_N2.map(e => new Escudo(e.x, e.y)),
    ];
}
...
onTeclaPulsada(function(e) {
    ...
    if (e.key === 'Enter' && estadoJuego === 'moneda' && estadoMoneda === 'resultado') {
        if (eleccionJugador === resultadoMoneda) {
            vidas = 1;
            jugador.x  = INICIO_N2.x;
            jugador.y  = INICIO_N2.y;
            jugador.vx = 0;
            jugador.vy = 0;
            monedaUsada  = true;
            estadoJuego  = 'jugando';
...
function frame(deltaTime) {
    ...
    // ── Puerta — llave + 100 puntos → redirige a mapa ────────
    ...
    if (condicionPuerta && seToca(jugador.x, jugador.y, jugador.w, jugador.h, puertaX, puertaY, CELDA, CELDA)) {
        // Desbloquear nivel 3 y minijuego 1 al completar nivel 2
        localStorage.setItem('nivel3_desbloqueado', 'true');
        localStorage.setItem('minijuego1_desbloqueado', 'true');
